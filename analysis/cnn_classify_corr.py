    ...
    for i in tqdm(range(1,60)):
        ...
        try:
            ...
            for j in range(len(data_score)):
                if data_score[j]>9.5:
                    datay.append(0)
                    c6=get_aeraeeg_corr(data_eeg[data_eeg["epoch"]==j][-3000:-2500])
                    c5=get_aeraeeg_corr(data_eeg[data_eeg["epoch"]==j][-2500:-2000])
                    c4=get_aeraeeg_corr(data_eeg[data_eeg["epoch"]==j][-2000:-1500])
                    c3=get_aeraeeg_corr(data_eeg[data_eeg["epoch"]==j][-1500:-1000])
                    c2=get_aeraeeg_corr(data_eeg[data_eeg["epoch"]==j][-1000:-500])
                    c1=get_aeraeeg_corr(data_eeg[data_eeg["epoch"]==j][-500:])
...
def train_model():
    ...
    """
    dataxy=[]
    with open("D:/code/code/eegemotion/git/model/corr_classify/corr_score_classify.txt","rb") as f:
    ...
    torch.save(model.state_dict(),"D:/code/code/eegemotion/git/model/corr_classify/model.pt")

def save_input_data():
    dataxy=get_data()
    with open("D:/code/code/eegemotion/git/model/corr_classify/corr_score_classify.txt","wb") as f:
        pickle.dump(dataxy,f)
